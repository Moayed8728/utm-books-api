...
final class BookRepository
{
    ...
    public function find(int $id): ?array {
        $stmt = $this->pdo->prepare('SELECT id, title, author, year, genre, created_by, created_at, updated_at FROM books WHERE id = :id');
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
    ...
    public function update(int $id, array $b): int {
        ...
        foreach (['title','author','genre'] as $f) {
            if (array_key_exists($f, $b)) { $sets[] = "$f=:$f"; $args[":$f"] = trim((string)$b[$f]); }
        }
